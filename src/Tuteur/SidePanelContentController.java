...
import Etudiant.InterfaceGlobaleEtudController;
...
public class SidePanelContentController implements Initializable {
    ...
    private void aide(ActionEvent event) throws IOException {
		  ...
		  JFXDialog dialog = new JFXDialog(InterfaceGlobaleTutController.stackPaneP, content, JFXDialog.DialogTransition.CENTER);
		  dialogP=dialog;
		  dialog.show();
    ...
    public void initialize(URL url, ResourceBundle rb) {
    	//change.setText(LoginController.username +" "+LoginController.name);
    }    
    ...
    private void clic(ActionEvent event) {
        ...
        switch(btn.getId())
        {
            case "b2":
            	...
            	InterfaceGlobaleTutController.lblTutP.setText("CONSULTATION DE MES AFFECTATIONS");
            	Pane contenureserv;
				try {
					contenureserv = FXMLLoader.load(getClass().getResource("contenuTuteur.fxml"));
					InterfaceGlobaleTutController.drawer3P.setSidePane(contenureserv);	             
					InterfaceGlobaleTutController.drawer3P.open();
				} catch (IOException e) {
					e.printStackTrace();
				}
            break;
